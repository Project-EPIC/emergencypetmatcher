...
def delete_PetReunion_images():
	delete_images(target_dir=PETREUNION_UPLOADS_DIRECTORY)
	delete_images(target_dir=PETREUNION_THUMBNAILS_DIRECTORY)
...
def create_random_User(i=0, pretty_name=True):
	if pretty_name == True:
...
def create_random_PetReunion(save=True, petreport=None, matched_petreport=None):
	if not petreport:
	...
	pr.description = generate_lipsum_paragraph(PETREUNION_DESCRIPTION_LENGTH)

	if random.random() <= 0.60:
		load_PetReport_sample_images()
		if pr.petreport.pet_type == PETREPORT_PET_TYPE_DOG:
			pr.set_images(MEDIA_ROOT + random.choice(PETREPORT_SAMPLE_DOG_IMAGES), save=save)
		elif pr.petreport.pet_type == PETREPORT_PET_TYPE_CAT:
			pr.set_images(MEDIA_ROOT + random.choice(PETREPORT_SAMPLE_CAT_IMAGES), save=save)
		elif pr.petreport.pet_type == PETREPORT_PET_TYPE_BIRD:
			pr.set_images(MEDIA_ROOT + random.choice(PETREPORT_SAMPLE_BIRD_IMAGES), save=save)
		elif pr.petreport.pet_type == PETREPORT_PET_TYPE_HORSE:
			pr.set_images(MEDIA_ROOT + random.choice(PETREPORT_SAMPLE_HORSE_IMAGES), save=save)
		elif pr.petreport.pet_type == PETREPORT_PET_TYPE_RABBIT:
			pr.set_images(MEDIA_ROOT + random.choice(PETREPORT_SAMPLE_RABBIT_IMAGES), save=save)
		elif pr.petreport.pet_type == PETREPORT_PET_TYPE_SNAKE:
			pr.set_images(MEDIA_ROOT + random.choice(PETREPORT_SAMPLE_SNAKE_IMAGES), save=save)
		elif pr.petreport.pet_type == PETREPORT_PET_TYPE_TURTLE:
			pr.set_images(MEDIA_ROOT + random.choice(PETREPORT_SAMPLE_TURTLE_IMAGES), save=save)
		else:
			pr.set_images(MEDIA_ROOT + PETREPORT_UPLOADS_DEFAULT_OTHER_IMAGE, save=save)
	else:
		pr.set_images(None, save=save)
	return pr


''' Function for setting up User (with passwords and optionally following lists), Client, PetReport, PetMatch, and PetReunion objects for testing purposes.'''
def setup_objects(delete_all_objects=True, create_users=True, num_users=NUM_USERS, create_following_lists=False, create_bookmark_lists = False, create_clients=True,
	create_petreports=False, num_petreports=NUM_PETREPORTS, create_petmatches=False, num_petmatches=NUM_PETMATCHES, allow_closed_matches=False, create_petreunions=False,
	num_petreunions=NUM_PETREUNIONS):
	...
	clients = [None for i in range (num_users)]
	...
	if create_petreunions == True:
		for i in range(num_petreunions):
			petreunions.append(create_random_PetReunion())
...
if __name__ == "__main__":
	...
	if argument == "setup":
		results = setup_objects(
			create_users=True,
			create_bookmark_lists=True,
			create_following_lists=True,
			create_petreports=True,
			create_petmatches=True,
			create_petreunions=True
		)

		print_info_msg("%d PetMatches generated" % NUM_PETMATCHES)
		print_info_msg("%d PetReports generated" % NUM_PETREPORTS)
		print_info_msg("%d PetReunions generated" % NUM_PETREUNIONS)
